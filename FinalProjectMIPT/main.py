...
file_operations_frame_load = Frame(window)
file_operations_frame_do = Frame(window)
file_operations_frame_save = Frame(window)
file_operations_frame_load.grid(row=0, column=0, sticky=EW, padx=5, pady=5)
file_operations_frame_do.grid(row=3, column=1, sticky=NS, padx=5, pady=5)
file_operations_frame_save.grid(row=4, column=0, sticky=EW, padx=5, pady=5)

file_operations_frame_do.rowconfigure(0, weight=1)
file_operations_frame_do.columnconfigure(0, weight=1)

load_image_button = ttk.Button(
    file_operations_frame_load,
    text="Загрузить изображение",
    command=on_load_image_button_click,
)
load_image_button.pack(fill=X, side=TOP, expand=True)
do_image_button = ttk.Button(
    file_operations_frame_do,
    text="Преобразовать изображение",
    command=on_do_image_button_click,
)
do_image_button.pack(fill=BOTH, side=TOP, expand=True)
save_image_button = ttk.Button(
    file_operations_frame_save,
    text="Сохранить изображение",
    command=on_save_image_button_click, )
save_image_button.pack(fill=X, side=TOP, expand=True)
